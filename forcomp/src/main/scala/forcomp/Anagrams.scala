...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match
      ...
      case head :: tail =>
        ...
        (for occu <- suboccurrences(head)
        yield tails ++ tails.map(tail => occu :: tail)).flatten
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match
    case Nil          => x
    case head :: tail => subtract(subtratcOccurrence(x, head), tail)

  def subtratcOccurrence(x: Occurrences, occu: (Char, Int)): Occurrences =
    (x, occu) match
      case (List(), _) => x
      case (head :: tail, occu) =>
        if head == occu then tail
        else if head._1 == occu._1 && head._2 != occu._2 then
          (head._1, head._2 - occu._2) :: tail
        else head :: subtratcOccurrence(tail, occu)
  ...
  def recur(freqs: Occurrences): List[Sentence] = freqs match
    case List() => List(List())
    case _ =>
      for
