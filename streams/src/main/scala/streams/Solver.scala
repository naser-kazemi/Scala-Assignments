...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(
    ...
    def neighborsWithHistoryAux(
        neighbors: List[(Block, Move)]
    ): LazyList[(Block, List[Move])] = neighbors match
      case Nil => LazyList.empty
      case (neighbor, move) :: tail =>
        (neighbor, move :: history) #:: neighborsWithHistoryAux(tail)
    neighborsWithHistoryAux(b.legalNeighbors)
  ...
  def newNeighborsOnly(
    ...
    neighbors.filter(newNeighbor => !explored.contains(newNeighbor._1))
