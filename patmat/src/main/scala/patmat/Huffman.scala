...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match
    case Nil => List()
    case ch :: chs =>
      val result: (Char, Int) = (ch, charTimes(chars, ch))
      result :: times(removeChar(chars, ch, Nil))

  def charTimes(chars: List[Char], ch: Char): Int =
    def calculate(chars: List[Char], ch: Char, count: Int): Int = chars match
      case Nil => count
      case x :: xs =>
        if x == ch then calculate(xs, ch, count + 1)
        else calculate(xs, ch, count)
    calculate(chars, ch, 0)

  def removeChar(chars: List[Char], ch: Char, acc: List[Char]): List[Char] =
    chars match
      case List() => acc
      case x :: xs =>
        if x != ch then removeChar(xs, ch, acc ::: List(x))
        else removeChar(xs, ch, acc)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match
    case left :: right :: xs =>
      (makeCodeTree(left, right) :: xs).sortWith((tree1, tree2) =>
